...
退出码（2026-09-09 外审 H1/H2 修复；2026-09-10 内部预筛复测再修 H2；2026-09-10 外审
H3 修复优先级顺序）：0=全部处理完毕（没有 tie/unknown/error，磁盘上最终也没有残留未
复核标记）；1=存在需要人工处理的词（并列 tie / 无法识别 unknown / 分析出错 error），
这些词不会被写回；2=用法错误；3=存在按"字位数最少"启发式选出、但尚未落盘的 resolved
候选（dry-run 或默认 --write，即未加 --write-heuristic 时命中）；4=本次运行结束后磁盘
上的最终文件内容里仍残留 @gen-segments-unreviewed 标记（不论这个标记是本次写入的，
还是更早一次 --write-heuristic 遗留、这次根本没碰到那些词——一旦词已经有 segments，
就不会再进入 resolveWord/writeSuggestions 的处理路径），未经人工复核（check_data.js
会拦截带此标记的数据）。1/2/3/4 都算失败退出（非 0），调用方按"非 0 即不可放行"处理
即可；细分只是给人读日志时定位问题严重程度用。3 与 4 的区别：3="候选还没被写进文件"
（该考虑要不要加 --write-heuristic 重跑），4="磁盘上还带着未复核标记，等着人复核"
（该去打开文件删标记做复核）——H1 修复的是"只含 resolved 的文件在任何模式下都得到
退出码 0"；H2 修复的是"--write-heuristic 写回未复核标记后，仍会得到退出码 0，与
check_data.js 会拦截同一份数据这件事矛盾"；2026-09-10 复测发现 H2 的实作只覆盖了
"本次运行写入"这一种情形，重跑（标记已经在磁盘上、这次根本没碰到那些词）恒返回 0，
改为直接扫描本次运行后磁盘上的最终文件内容判定退出码 4，三种模式（dry-run/--write/
--write-heuristic）统一走这条判据。

**判断优先级（2026-09-10 外审 H3 修复）：磁盘有未复核标记恒 4，其次 1，再次 3。**
标记是跨次运行持久的磁盘状态，1/3 只是本次运行的局部状态——旧实现按 1 → 3 → 4 的
顺序判断，会让"标记 + tie/unknown/error 同时存在"命中 1、"标记 + 未写候选同时存在"
命中 3，4 因此永远轮不到，与上一段"只要磁盘还有标记，重跑必须一直是 4"的承诺矛盾。
