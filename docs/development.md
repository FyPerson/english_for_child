...
5. `python tools/project.py serve`：构建并打包后在本机 `127.0.0.1:8000` 查看发布目录，不跑回归。固定端口有助于复用同一来源下的进度；更换端口会形成另一来源。
...
`python tools/project.py release` 先跑完整回归（其第一步就是构建 `build/`），通过后直接打包 `build/`，不再另行构建；`build/` 里有白名单之外的文件即拒绝打包。失败不继续打包。目录使用本机时间命名，如 `soundblocks-2026-09-05_14-30-00`；同秒有不同内容时追加序号，内容相同则复用已有目录。内容标识与每个文件的哈希仍记录在 `manifest.json`。`dist/最新版本.txt` 指向最近打包的目录（包括本地预览打包）。
