...
def run():
    with sync_playwright() as p:
        ...
        assert not page.evaluate('CourseGate.allowed(1,2)')
        ...
        assert page.evaluate('CourseGate.allowed(1,2)')
        ...
        try:
            ...
            web.evaluate('''()=>{DAYS[0].steps.forEach((s,si)=>s.blocks.filter(b=>b.b==='checks').forEach(b=>b.items.forEach((_,i)=>dayState(1).checks[`1-${si}-${i}`]=true)));save();}''')
