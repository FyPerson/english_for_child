...
const RESERVED = ['ram','hem','rid','dam','kid'];   /* 周检五词，全 CVC，本周任何练习/游戏/小书里都不出现 */
...
const FIRST_TEACH_DAY = { c:1, k:1, e:2, h:3, r:4, m:5, d:6 };   /* c 和 k 同一天教，同一个音两件外套 */
...
const G1_THEME = {
  c: { title:'轮 A · 抓住 /k/', icon:'camera', cmd:'开头听到 k 就拍气球' },
  e: { title:'轮 B · 抓住 /e/', icon:'huh', cmd:'听到词的肚子里藏着 /e/ 就拍气球' }
};
...
const DAYS = [
{
  ...
  steps:[
    ...
    {t:'声音抓抓乐', min:5, blocks:[
      {b:'lead', html:'和第一周第一天一样，今天也用耳朵收尾。<b>电脑会念词，孩子听到开头是 /k/ 的就拍气球。</b>先跟他把口令说一遍再开始。'},
      {b:'g1', only:'c'},
      {b:'note', tone:'bulb', html:'注意听 <span class="en">cat</span> 和 <span class="en">kit</span>——<b>开头是同一个 /k/，但写出来一个是 c 一个是 k</b>，正好印证今天那件「两件外套」的事。<br>拍错很正常，不用评判对错，游戏自己会记分。'}
    ]},
    {t:'收尾打卡', min:3, blocks:[
      {b:'checks', items:[
